...
class NSStateMap {
	...
	static inline size_t getP(size_t n0, size_t n1, size_t shift) {
		if (!n0 && !n1) return 1 << (shift - 1);
		return (n0 << shift) / (n0 + n1);
	}

	size_t find_state(size_t best, byte* n, size_t b) const {
		// How to compare?? hmmm
		size_t o = b^1;
		size_t p = getP(n[0], n[1], 16);
		for (size_t i = 0;i < 256;++i) {
			const State& st = states[i];
			size_t stp = getP(st.n[0], st.n[1], 16);
...
public:
	...
	static const size_t pmax = 1 << pshift;
	...
	inline size_t p(size_t state) const {
		return states[state].p;
	}

	inline size_t getTransition(const size_t state, size_t bit) {
		return states[state].t[bit];
	}

	inline size_t next(const size_t state, size_t bit) {
		if (count_probs) {
			++states[state].np[bit];
		}
		return states[state].t[bit];
	...
	inline size_t total(size_t state) const {
		return states[state].tot;
	}

	void dump(std::ostream& sout) {
		for (size_t i = 0; i < num_states; ++i) {
			sout << size_t(pmax * double(states[i].np[0]) / double(states[i].np[0] + states[i].np[1])) << ",";
		}
		sout << std::endl;
		size_t unused = 0;
		for (size_t i = 0; i < num_states; ++i) {
	...
	size_t optimizeStatesRec(size_t st, StateReMap& sm, StateVec& new_states) {
		auto found = sm.find(st);
		if (found == sm.end()) {
			sm.insert(std::make_pair(st, new_states.size()));
			new_states.push_back(std::make_pair(size_t(0), size_t(0)));
